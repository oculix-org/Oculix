...
class PatternPaneTargetOffset extends JPanel implements
  ...
  void findTarget(final String patFilename, final Location initOffset) {
    new Thread(() -> {
      try {
        // Finder requiert OpenCV natif — on tente, on ignore si indispo
        ...
        // OpenCV non disponible ou autre erreur — on charge juste l'image
        Debug.error(me + "findTarget: OpenCV indispo ou erreur: %s", e.getMessage());
        EventQueue.invokeLater(() -> {
          try {
            _img = ImageIO.read(new File(patFilename));
  ...
  private void changeSize(int zoomInOut) {
    // _img pas encore charge — on ignore
    if (_img == null) {
      return;
    }
    resizableRect.setVisible(false);
  ...
  public void mouseReleased(MouseEvent me) {
    if (me.getComponent() == resizableRect) {
      ...
      changedBounds = new Rectangle(center.getX(), center.getY(),
              (int) ((bounds.width - STROKE_WIDTH * 2) / _zoomRatio),
              (int) ((bounds.height - STROKE_WIDTH * 2) / _zoomRatio));
  ...
  public void paintComponent(Graphics g) {
    ...
    if (getWidth() > 0 && getHeight() > 0) {
      if (_match != null) {
        ...
        // _img pas encore charge — on peint juste le fond
        if (_img == null) {
          paintBackground(g2d);
        } else {
  ...
  void paintMatch(Graphics2D g2d) {
    // _img garanti non null ici (appelé seulement si _img != null dans paintComponent)
    ...
    if (_match != null) {
      if (!resizableRect.isVisible()) {
        ...
        resizableRect.setMaxBounds(new Rectangle(
                point.x - STROKE_WIDTH, point.y - STROKE_WIDTH,
                (int) (_simg.getImage().getWidth() * _zoomRatio) + STROKE_WIDTH * 2,
                (int) (_simg.getImage().getHeight() * _zoomRatio) + STROKE_WIDTH * 2));

        if (changedBounds != null) {
          point = this.convertScreenToView(new Location(changedBounds.x, changedBounds.y));
          resizableRect.setBounds(point.x - STROKE_WIDTH, point.y - STROKE_WIDTH,
                  (int) (changedBounds.width * _zoomRatio) + STROKE_WIDTH * 2,
                  (int) (changedBounds.height * _zoomRatio) + STROKE_WIDTH * 2);
        } else {
          resizableRect.setBounds(x - STROKE_WIDTH, y - STROKE_WIDTH,
