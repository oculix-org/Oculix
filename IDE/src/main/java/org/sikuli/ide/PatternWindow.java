...
public class PatternWindow extends JFrame {
  ...
  private PatternPaneNaming _paneNaming;
  ...
  private void init() {
    ...
    paneNaming = createNamingPanel();
    tabPane.addTab("Fichier", paneNaming);

    // onglet 1 : Prévisualiser les correspondances
    tabSequence++;
    msgApplied[tabSequence] = new JLabel();
    setMessageApplied(tabSequence, false);
    ...
    try {
      _screenshot.setParameters(imgBtn.getFilename(), false, 0.7, PatternPaneScreenshot.MAX_NUM_MATCHING);
    } catch (Exception e) {
  ...
  //<editor-fold desc="005 naming panel">
  private JPanel createNamingPanel() {
    JPanel p = new JPanel();
    p.setLayout(new BoxLayout(p, BoxLayout.Y_AXIS));
    _paneNaming = new PatternPaneNaming(imgBtn, msgApplied[tabSequence]);
    createMarginBox(p, _paneNaming);
    p.doLayout();
    return p;
  }
  //</editor-fold>
  ...
  private void actionPerformedUpdates(Window _parent) {
    ...
    // renommage depuis onglet Fichier
    if (_paneNaming.isDirty()) {
      String filename = _paneNaming.getAbsolutePath();
      ...
      try {
        ...
        Debug.error("renaming failed: old: %s \nnew: %s\n%s", oldFilename, filename, ioe.getMessage());
    ...
    if (changedBounds != null) {
      File file = new File(_paneNaming.getAbsolutePath());
      BufferedImage changedImg = _simg.getImage().getSubimage(
          changedBounds.x, changedBounds.y, changedBounds.width, changedBounds.height);
      try {
  ...
  class ActionApply implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (tabPane != null && tabPane.getSelectedComponent() != null) {
        tabPane.getSelectedComponent().transferFocus();
      }
    }
  }
      ...
      if (isDirty()) {
        ...
        if (isFileOverwritten) {
          try {
            ...
            Debug.error("revert renaming failed: %s", ioe.getMessage());
          }
        }
      }
      close();
